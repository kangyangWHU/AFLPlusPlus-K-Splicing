...
static void usage(u8 *argv0, int more_help) {
  ...
  SAYF(
      ...
      "bytes.\n"
      "  -k            - k-crossover in splice stage: k=0, disable splice\n"
      "  -K            - Sample k targets and keep the target with highest perf score\n\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
                       "+a:Ab:B:c:CdDe:E:f:F:g:G:hi:I:l:k:K:L:m:M:nNo:Op:P:QRs:S:t:"
                       "T:UV:WXx:YZ")) > 0) {

    switch (opt) {

      case 'k':
        afl->k_splice = atoi(optarg);
        break;
      case 'K':
        afl->k_target = atoi(optarg);
        break;
