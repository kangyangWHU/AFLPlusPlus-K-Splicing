...
retry_splicing:
  ...
  //  afl->k_splice == 0 will disable splice
  if (afl->use_splicing && splice_cycle++ < SPLICE_CYCLES &&
    ...
    u8                 *new_buf;
    s32                 f_diff, l_diff;
    u32 *split_at_list = (u32*) malloc(afl->k_splice * sizeof(u32));
    ...
    // select the best target among K times sample
    int best_perf_score = 0;
    u32 best_tid = 0;
    for(int i=0; i<afl->k_target;i++) {
      ...
      if(afl->queue_buf[tid]->perf_score > best_perf_score){
        best_perf_score = afl->queue_buf[tid]->perf_score;
        best_tid  = tid;
      }
    ...
    /* Get the testcase */
    afl->splicing_with = best_tid;
    target = afl->queue_buf[best_tid];
    new_buf = queue_testcase_get(afl, target);
    ...
    for(int i=0; i<afl->k_splice; i++){
        split_at = f_diff + rand_below(afl, l_diff - f_diff);
        split_at_list[i] = split_at;
    }

  // sort the split point
    int n = afl->k_splice;
    for (int i = 0; i < n - 1; i++) {
      for (int j = 0; j < n - i - 1; j++) {
        if (split_at_list[j] > split_at_list[j + 1]) {
          // Swap arr[j] and arr[j + 1]
          int temp = split_at_list[j];
          split_at_list[j] = split_at_list[j + 1];
          split_at_list[j + 1] = temp;
        }
      }
    }
...
//    len = target->len;
    ...
    for(int i=0; i <= afl->k_splice; i++)
    {
      ...
      if(i % 2 ==0)
      {
        memcpy(afl->in_scratch_buf + temp_start, in_buf+temp_start, temp_end - temp_start);
      }else
      {
        memcpy(afl->in_scratch_buf + temp_start, new_buf+temp_start, temp_end - temp_start);
      }

      if(i < afl->k_splice)
      {
        temp_start = split_at_list[i];
      }
